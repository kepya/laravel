    ...
    public function addOneInvoice()
    {
        ...
        if ($response1['status'] == 200) {
            ...
            for ($i=0; $i < $length; $i++) {
                $curl = curl_init();
                curl_setopt_array($curl, array(
                    CURLOPT_URL => 'http://localhost:4000/admin/facture/haveInvoice/' . $users[$i] -> _id,
                    CURLOPT_RETURNTRANSFER => true,
                    CURLOPT_ENCODING => '',
                    CURLOPT_MAXREDIRS => 10,
                    CURLOPT_TIMEOUT => 0,
        ...
        if (isset($_POST['submit'])) {
            ...
            if(array_key_exists('result',$response)){
                ...
                if (!empty($response['result'])) {
                    ...
                    $url = curl_init();
                    curl_setopt_array($url, array(
                        CURLOPT_URL => 'http://localhost:4000/admin/facture/userThatHaveNotPaidInvoiceWithDate/' . $date,
                        CURLOPT_RETURNTRANSFER => true,
                        CURLOPT_ENCODING => '',
                        CURLOPT_MAXREDIRS => 10,
                        CURLOPT_TIMEOUT => 0,
